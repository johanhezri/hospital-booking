...
async handleDailyReminders() {
    ...
    const appointments = await this.appointmentsService.findBookedBetween(nextDayStart, nextDayEnd);
    ...
    for (const appt of appointments) {
      try {
        ...
        const localTime = DateTime.fromJSDate(appt.starts_at, { zone: 'utc' }).setZone(tz);

        await this.mailService.sendMail({
          hospital,
          to: 'johan.hezri@gmail.com', // TODO get patient's email
          subject: `Appointment Reminder with Dr. ${appt.doctor_id}`,
          text: `Hi ${appt.patient_id}, you have an appointment tomorrow at ${localTime.toFormat('HH:mm')} (${tz}).`,
        });

        this.logger.log(`Reminder sent for appointment ${appt.id} ('johan.hezri@gmail.com').`);
      } catch (err) {
        this.logger.error(`Failed to send reminder for appointment ${appt.id}: ${err.message}`);
      }
    }
